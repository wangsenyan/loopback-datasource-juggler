...
if (Transaction) {
  ...
  Transaction.prototype.commit = function(cb) {
    cb = cb || utils.createPromiseCallback();
    //mongo transaction use
    if (self.isInTransation) {
      self.session.commitTransaction(function (e, r) {
        if (e) return cb(e);
        self.session.isInTransation = false;
        self.session.endSession(cb);
      })
    }else{
  ...
  Transaction.prototype.rollback = function(cb) {
    cb = cb || utils.createPromiseCallback();
    if (self.isInTransation) {
      self.session.abortTransaction(function (e, r) {
        if (e) return cb(e);
